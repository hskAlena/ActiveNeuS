...
class Pipeline(nn.Module):
    ...
    def get_train_loss_dict(self, step: int):
        ...
        model_outputs = self.model(ray_bundle, batch, steps=step)
        metrics_dict = self.model.get_metrics_dict(model_outputs, batch, steps=step)
        loss_dict = self.model.get_loss_dict(model_outputs, batch, metrics_dict, steps=step)
...
class VanillaPipeline(Pipeline):
    ...
    def get_eval_loss_dict(self, step: int):
        ...
        self.eval()
        ray_bundle, batch = self.datamanager.next_eval(step)
        model_outputs = self.model(ray_bundle, steps=step)
        metrics_dict = self.model.get_metrics_dict(model_outputs, batch)
        loss_dict = self.model.get_loss_dict(model_outputs, batch, metrics_dict)
        self.train()
        return model_outputs, loss_dict, metrics_dict
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        # import pdb
        # pdb.set_trace()
        outputs = self.model.get_outputs_for_camera_ray_bundle(camera_ray_bundle, steps=step)
    ...
    def get_average_eval_image_metrics(self, step: Optional[int] = None):
        ...
        with Progress(
            ...
            view_no = 0
            eval_images = self.datamanager.eval_dataset._dataparser_outputs.image_filenames
            for camera_ray_bundle, batch in self.datamanager.fixed_indices_eval_dataloader:
                ...
                if 'dtu' in datapath:
                    target_no = int(str(eval_images[view_no]).split('/')[-1].split('_')[0])
                else:
                    target_no = 0
                metrics_dict, images_dict = self.model.get_image_metrics_and_images( outputs, batch, target_no, datapath, cameras)
    ...
    @profiler.time_function
    def get_average_eval_image_metrics_original(self, step: Optional[int] = None):
        """Iterate over all the images in the eval dataset and get the average.
