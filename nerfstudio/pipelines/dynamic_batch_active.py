...
@dataclass
class DynamicBatchActivePipelineConfig(VanillaPipelineConfig):
    """Dynamic Batch Pipeline Config"""

    _target: Type = field(default_factory=lambda: DynamicBatchActivePipeline)
...
class DynamicBatchActivePipeline(VanillaPipeline):
    """Pipeline with logic for changing the number of rays per batch."""

    # pylint: disable=abstract-method

    config: DynamicBatchActivePipelineConfig
    ...
    def __init__(
        ...
        ), "DynamicBatchPipeline only works with ActiveDataManager."
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if (step+1) % self.config.topk_iter == 0:
            topk_list = []
            ...
            if select_scheme == 'frontier':
                for i in range(len(candidates)):
                    ray_bundle, batch = self.datamanager.train_candidate_iter(i)
                    topk_list.append(self._model.eval_k_views(ray_bundle))
                topk_add = self._model.choose_k_views(candidates, topk_list, candidate_cam_info, 
