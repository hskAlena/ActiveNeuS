...
from nerfstudio.models.neus_acc import NeuSAccModel, NeuSAccModelConfig
from nerfstudio.utils import colormaps


@dataclass
class NeuralangeloAccModelConfig(NeuSAccModelConfig):
    """Neuralangelo Model Config"""

    _target: Type = field(default_factory=lambda: NeuralangeloAccModel)
...
class NeuralangeloAccModel(NeuSAccModel):
    """Neuralangelo model
    ...
    config: NeuralangeloAccModelConfig
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict:
        ...
        if self.training and self.config.curvature_loss_multi > 0.0:
            delta = self.field.numerical_gradients_delta # 0.125
            if (outputs['field_outputs'] is None) or (FieldHeadNames.SDF not in outputs['field_outputs'].keys()):
                # loss_dict["curvature_loss"] = torch.zeros(1, device=outputs['rgb'].device)
                return loss_dict
            centered_sdf = outputs['field_outputs'][FieldHeadNames.SDF]
            sourounding_sdf = outputs['field_outputs']["sampled_sdf"]
            # centered_sdf shape [512, 128, 1]
            # sourounding_sdf shape [512, 128, 6]
            
            if len(centered_sdf.shape) <3:
                sourounding_sdf = sourounding_sdf.reshape(centered_sdf.shape[:1] + (3, 2))
            else:
