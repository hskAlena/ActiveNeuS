...
class SDFStudio(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):  # pylint: disable=unused-argument,too-many-statements
        ...
        datapath = str(self.config.data)
        # subsample to avoid out-of-memory for validation set
        if split !='train' and 'dtu' in datapath:
            indices = [1, 9, 12, 15, 24, 27, 32, 35, 42, 46]
        # elif 'dtu' in datapath:
        #     indices = [i for i in indices if i not in [1, 9, 12, 15, 24, 27, 32, 35, 42, 46] ]
        # elif split != "train" and self.config.skip_every_for_val_split >= 1:
