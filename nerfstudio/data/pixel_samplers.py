...
def collate_image_dataset_batch(batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False,
                                precrop_iters=0, precrop_frac=0.5, steps=None):
    """
    ...
    if steps is not None and steps<precrop_iters:
        image_height = int(image_height*precrop_frac)
        image_width = int(image_width*precrop_frac)

    # only sample within the mask, if the mask is in the batch
    if "mask" in batch:
        mask_single_channel = batch['mask'][..., 0, 0]
        nonzero_indices = torch.nonzero(mask_single_channel, as_tuple=False)
        chosen_indices = random.sample(range(len(nonzero_indices)), k=num_rays_per_batch)
        # print(f"{nonzero_indices.shape=} {len(chosen_indices)=}")
...
class PixelSampler:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, num_rays_per_batch: int, keep_full_image: bool = False,
    ...
    def sample(self, image_batch: Dict, steps=None):
        ...
        if isinstance(image_batch["image"], list):
            ...
            pixel_batch = collate_image_dataset_batch_list(
                ...
                precrop_iters=self.precrop_iters, precrop_frac=self.precrop_frac, steps=steps
            )
