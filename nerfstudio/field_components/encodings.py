...
from nerfstudio.utils.math_nerf import components_from_spherical_harmonics, expected_sin
...
def lossfun_distortion(t, w):
  ...
  ut = (t[..., 1:] + t[..., :-1]) / 2
  dut = np.abs(ut[..., :, None] - ut[..., None, :])
  loss_inter = np.sum(w * np.sum(w[..., None, :] * dut, axis=-1), axis=-1)

  # The loss incurred within each individual interval with itself.
  loss_intra = np.sum(w**2 * (t[..., 1:] - t[..., :-1]), axis=-1) / 3

  return loss_inter + loss_intra

def get_freq_reg_mask(pos_enc_length, current_iter, total_reg_iter, start_reg_iter, max_visible=None, type='submission'):
  '''
  ...
  if max_visible is None:
    # default FreeNeRF
    if current_iter >= start_reg_iter and current_iter < total_reg_iter:
...
# def get_freq_reg_grad(pos_enc_length, current_iter, total_reg_iter, max_visible=None, type='submission'):
#   '''
#   Returns a frequency mask for position encoding in NeRF.
...
#       freq_mask = torch.zeros(pos_enc_length)  # all invisible
#       ptr = pos_enc_length / F * current_iter / total_reg_iter + 1 
#       ptr = ptr if ptr < pos_enc_length / F else pos_enc_length / F
#       int_ptr = int(ptr)
#       freq_mask[: int_ptr * F] = 1.0  # assign the integer part
#       freq_mask[int_ptr * F : int_ptr * F + F] = (ptr - int_ptr)  # assign the fractional part
#       return torch.clip(torch.Tensor(freq_mask), 1e-8, 1-1e-8)  # for numerical stability
#     else:
#       return torch.ones(pos_enc_length)
#   else:
...
#     freq_mask[: int(pos_enc_length * max_visible)] = 1.0
#     return torch.Tensor(freq_mask)
def occ_reg_nerfacc(rgb, packed_info, density, reg_range=10, wb_prior=False, wb_range=20):
    '''
    Computes the occulusion regularization loss.
    Args:
        rgb (jnp.array): The RGB rays/images. [512, 2]
        density (jnp.array): The current density map estimate. [4503, 1]
    ...
    rgb_mean = torch.unsqueeze(rgb_mean, 1).repeat(1,density.shape[1],1)
...
def lossfun_occ_reg(rgb, density, reg_range=10, wb_prior=False, wb_range=15):
    '''
    Computes the occulusion regularization loss.
    Args:
