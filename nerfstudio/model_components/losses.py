...
class ScaleAndShiftInvariantLoss(nn.Module):
    ...
    def forward(self, prediction, target, mask):
        ...
        '''
        (Pdb) scale
        tensor([0.0024], device='cuda:0', grad_fn=<IndexPutBackward0>)
        (Pdb) shift
        tensor([1.9313], device='cuda:0', grad_fn=<IndexPutBackward0>)
        '''
...
class VarianceLoss(nn.Module):
    ...
    def __init__(self, rgb_w, uncert_w, density_w, var_eps=1e-6):
        super(VarianceLoss, self).__init__()
        # self.lamb = lamb
        self.var_eps = var_eps
        self.uncert_w = uncert_w
        self.rgb_w = rgb_w
        self.density_w = density_w

    def forward(self, image, outputs, fg_flag=False, fg_mask=None):
        """take the mim
        ...
        """
        d = {}
        var = outputs['uncertainty']

        sum_rgb_fine = (outputs['rgb'] - image) ** 2
        if fg_flag:
            mask_sum = fg_mask.sum() + 1e-5
            sum_rgb_fine = sum_rgb_fine * fg_mask
            d['rgb'] = self.rgb_w*torch.sum((sum_rgb_fine) / (2 * (var + self.var_eps)))/mask_sum
        else:
            d['rgb'] = self.rgb_w*torch.mean((sum_rgb_fine) / (2 * (var + self.var_eps)))
        if 'mean_sigma' in outputs.keys():
            d['density'] = self.density_w*torch.mean(outputs['mean_sigma'])

        # d['sigma'] = self.lamb * outputs['mean_sigma'].detach() + 4.0
        d['uncertainty'] = self.uncert_w*torch.mean(torch.log(var+ self.var_eps)) + 4.0
        # import pdb
        # pdb.set_trace()
...
class BoundLoss(nn.Module):
    ...
    def __init__(self, bounds_method='ray', trunc_distance=50.0, trunc_weight=5.0, gradient_weight = 0.02,
                 loss_type='L1'):
    ...
    def forward(self, sdf, sdf_grad, dirs_C_sample, depth_sample, T_WC_sample,
                z_vals, pc, norm_sample, t, trunc_weight=0.5, loss_type='L1'):
        """take the mim
        ...
        Returns:
            ...
            sdf_loss: sdf loss
        """
        bounds, grad_vec = self.bounds(self.bounds_method,
                                       dirs_C_sample=dirs_C_sample,
                                       depth_sample=depth_sample,
                                       T_WC_sample=T_WC_sample,
                                       z_vals=z_vals,
                                       pc=pc,
                                       normal_trunc_dist=self.trunc_distance,
    ...
    def bounds(
        ...
        if method == "ray":
            bounds, grad = self.bounds_ray(
                depth_sample, z_vals, dirs_C_sample, T_WC_sample, do_grad
            )

        elif method == "normal":
            bounds, grad = self.bounds_normal(
                depth_sample, z_vals, dirs_C_sample,
                norm_sample, normal_trunc_dist, T_WC_sample, do_grad)

        else:
            bounds, grad = self.bounds_pc(pc, z_vals, depth_sample, do_grad)

        return bounds, grad
    
    
    
    def bounds_ray(self, depth_sample, z_vals, dirs_C_sample, T_WC_sample, do_grad):
        ...
        grad = None
        if do_grad:
            grad = self.grad_ray(T_WC_sample, dirs_C_sample, z_vals.shape[1] - 1)

        return bounds, grad


    def bounds_normal(self, 
        depth_sample, z_vals, dirs_C_sample, norm_sample, normal_trunc_dist,
        T_WC_sample, do_grad,
    ):
        ray_bounds = self.bounds_ray(depth_sample, z_vals, dirs_C_sample)
    ...
    def grad_ray(self, T_WC_sample, dirs_C_sample, n_samples):
        """ Returns the negative of the viewing direction vector """
        _, dirs_W = self.origin_dirs_W(T_WC_sample, dirs_C_sample)
        grad = - dirs_W[:, None, :].repeat(1, n_samples, 1)

        return grad


    def bounds_pc(self, pc, z_vals, depth_sample, do_grad=True):
