...
from nerfstudio.utils.math_nerf import components_from_spherical_harmonics
...
class RGBRenderer(nn.Module):
    ...
    def combine_rgb(
        ...
        if ray_indices is not None and num_rays is not None:
            ...
            comp_rgb = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=rgb, n_rays=num_rays)
            accumulated_weight = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=None, n_rays=num_rays)
        else:
...
class AccumulationRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            accumulation = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=None, n_rays=num_rays)
        else:
...
class DepthRenderer(nn.Module):
    ...
    def forward(
        ...
        if self.method == "expected":
            ...
            if ray_indices is not None and num_rays is not None:
                # Necessary for packed samples from volumetric ray sampler
                depth = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=steps, n_rays=num_rays)
                accumulation = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=None, n_rays=num_rays)
...
class UncertaintyRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            uncertainty = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=betas, n_rays=num_rays)
        else:
...
class SemanticRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            sem = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=semantics, n_rays=num_rays)
        else:
...
class NormalsRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            n = nerfacc.accumulate_along_rays(weights, ray_indices=ray_indices, values=normals, n_rays=num_rays)
        else:
