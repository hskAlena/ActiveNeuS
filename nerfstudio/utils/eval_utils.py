...
def eval_setup(
    ...
    # config.pipeline.datamanager.dataparser.load_dtu_highres = True
    config.trainer.load_dir = config.get_checkpoint_dir()
    tmp_loaddir = '/'.join(str(config.trainer.load_dir).split('/')[:-1])
    model_dir = str(config.trainer.load_dir).split('/')[-1]
    config_tmp = '/'.join(str(config_path).split('/')[:-1])
    if tmp_loaddir != config_tmp:
        config.trainer.load_dir = Path('/'.join([config_tmp, model_dir]))
