...
@check_main_thread
def put_graph(name: str, inputs, step: int):
    """Setter function to place a dictionary of scalars into the queue to be written out
    ...
    Args:
        ...
        step: step associated with dict
    """
    EVENT_STORAGE.append({"name": name, "write_type": EventType.GRAPH, "event": inputs, "step": step})
...
class Writer:
    ...
    @abstractmethod
    def write_graph(self, name: str, inputs, step: int) -> None:
        """Required method to write a single scalar value to the logger
