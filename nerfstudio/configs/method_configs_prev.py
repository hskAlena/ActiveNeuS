...
#############################################################################################
method_configs["active-instant-ngp"] = Config(
    method_name="active-instant-ngp",
    trainer=TrainerConfig(
        steps_per_eval_batch=5000,
        steps_per_eval_image=5000,
        steps_per_save=20000,
    ...
    pipeline=DynamicBatchActivePipelineConfig(
        datamanager=ActiveDataManagerConfig(dataparser=NerfstudioDataParserConfig(), train_num_rays_per_batch=512), #8192),
        model=InstantNGPModelConfig(render_step_size=0.005, eval_num_rays_per_chunk=512), #8192),
    ),
    optimizers={
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=1e-2, eps=1e-15),
...
method_configs["active-neuralangelo"] = Config(
    method_name="active-neuralangelo",
    trainer=TrainerConfig(
        steps_per_eval_image=5000,
        steps_per_eval_batch=5000,
        steps_per_save=20000,
        steps_per_eval_all_images=1000000,  # set to a very large model so we don't eval with all images
        max_num_iterations=100_001, #500_001,
        mixed_precision=False,
    ),
    pipeline=ActivePipelineConfig(
        ...
        model=NeuralangeloModelConfig(
            sdf_field=SDFFieldConfig(
                ...
                base_res=16, #64,
                max_res=2048, #4096,
                log2_hashmap_size=18, #22,
                hash_features_per_level=4, #8,
                hash_smoothstep=False,
                use_position_encoding=False,
            ),
            background_model="mlp",
    ...
    optimizers={
        "fields": {
            ...
            # "scheduler": NeuSSchedulerConfig(warm_up_end=5000, learning_rate_alpha=0.05, max_steps=500000),
            #"scheduler": NeuSSchedulerConfig(warm_up_end=500, learning_rate_alpha=0.05, max_steps=20000),
            "scheduler": MultiStepWarmupSchedulerConfig(warm_up_end=500, milestones=[12000, 16000], gamma=0.1),
        },
        "field_background": {
            "optimizer": AdamWOptimizerConfig(lr=1e-3, eps=1e-15),
            "scheduler": MultiStepWarmupSchedulerConfig(warm_up_end=500, milestones=[12000, 16000], gamma=0.1),
...
method_configs["active-neuralangelo-acc"] = Config(
    method_name="active-neuralangelo",
    trainer=TrainerConfig(
        steps_per_eval_image=5000,
        steps_per_eval_batch=5000,
...
method_configs["active-neus"] = Config(
    method_name="active-neus",
    trainer=TrainerConfig(
        steps_per_eval_image=500,
        steps_per_eval_batch=5000,
        steps_per_save=20000,
    ...
    pipeline=ActivePipelineConfig(
        ...
        model=NeuSModelConfig(eval_num_rays_per_chunk=512), #1024),
    ),
    optimizers={
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=5e-4, eps=1e-15),
...
method_configs["uncert-neus"] = Config(
    method_name="uncert-neus",
    trainer=TrainerConfig(
        steps_per_eval_image=500,
        steps_per_eval_batch=5000,
        steps_per_save=20000,
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
            dataparser=SDFStudioDataParserConfig(),
            train_num_rays_per_batch=512, #1024,
            eval_num_rays_per_batch=512, #1024,
            camera_optimizer=CameraOptimizerConfig(
                mode="off", optimizer=AdamOptimizerConfig(lr=6e-4, eps=1e-8, weight_decay=1e-2)
            ),
        ),
        model=UncertNeuSModelConfig(eval_num_rays_per_chunk=512), #1024),
    ),
    optimizers={
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=5e-4, eps=1e-15),
...
method_configs["uncert_neus-acc"] = Config(
    method_name="uncert_neus-acc",
    trainer=TrainerConfig(
        steps_per_eval_image=5000,
        steps_per_eval_batch=5000,
        steps_per_save=20000,
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
            dataparser=SDFStudioDataParserConfig(),
            train_num_rays_per_batch=1024, #2048,
            eval_num_rays_per_batch=1024, #1024,
            camera_optimizer=CameraOptimizerConfig(
                mode="off", optimizer=AdamOptimizerConfig(lr=6e-4, eps=1e-8, weight_decay=1e-2)
            ),
        ),
        ## TODO check sdf field conf, use_grid_feature True
        model=ActiveNeuSAccModelConfig(eval_num_rays_per_chunk=512), # 1024),
    ),
    optimizers={
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=5e-4, eps=1e-15),
