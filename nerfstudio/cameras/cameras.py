...
class Cameras(TensorDataclass):
    ...
    def get_image_coords(
        self, pixel_offset: float = 0.5, index: Optional[Tuple] = None, valid=False
        ...
        if self.precrop_iters !=0 and valid:
            image_height = torch.div(image_height, 2, rounding_mode='trunc')
            image_width = torch.div(image_width, 2, rounding_mode='trunc')
            dH = int(image_height * self.precrop_frac)
            dW = int(image_width * self.precrop_frac)
            image_coords = torch.stack(
                            torch.meshgrid(
                                torch.linspace(image_height - dH, image_height + dH - 1, 2*dH), 
                                torch.linspace(image_width - dW, image_width + dW - 1, 2*dW)
                            ), -1) + pixel_offset
        else:
    ...
    def generate_rays(  # pylint: disable=too-many-statements
        ...
        if cameras.is_jagged and coords is None and (keep_shape is None or keep_shape is False):
            ...
            _coords = [cameras.get_image_coords(index=tuple(index), valid=valid).reshape(-1, 2) for index in camera_indices]
        ...
        if coords is None:
            ...
            coords: torch.Tensor = cameras.get_image_coords(index=tuple(index), valid=valid)  # (h, w, 2)
