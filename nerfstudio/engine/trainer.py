...
class Trainer:
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            step = 0
            # tmp_ray_bundle, tmp_batch = self.pipeline.datamanager.train_candidate_iter(0)
            # import pdb
            # pdb.set_trace()
            # writer.put_graph(name= "graph", inputs=[self.pipeline._model, tmp_ray_bundle], step=0)
            # writer.write_out_graph()
            # TODO Why  self._start_step + num_iterations
    ...
    def _load_checkpoint(self) -> None:
        ...
        if load_dir is not None:
            ...
            if self.pipeline.datamanager.train_candidate is not None:
                if type(loaded_state['i_train']) == set:
                    loaded_state['i_train'] = list(loaded_state['i_train'])
                    loaded_state['train_candidate'] = list(loaded_state['train_candidate'])
                self.pipeline.datamanager.train_image_dataloader.train_candidate = loaded_state['train_candidate']
                self.pipeline.datamanager.train_image_dataloader.i_train = loaded_state['i_train']
            ...
            torch.save(
                {
                    ...
                    "i_train": self.pipeline.datamanager.train_image_dataloader.i_train, 
                    "train_candidate": self.pipeline.datamanager.train_image_dataloader.train_candidate, 
                },
                ckpt_path,
            )
        else:
