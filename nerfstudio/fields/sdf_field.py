...
class SDFFieldConfig(FieldConfig):
    ...
    """whether to use uncertainty network"""
...
class SDFField(Field):
    ...
    def __init__(
        ...
        # TODO uncertainty network 
        self.uncertainty_net = uncertainty_net
        if self.uncertainty_net:
            self.uncertainty_net = nn.Linear(self.config.geo_feat_dim, 1)
        ...
        self.softplus_uncert = nn.Softplus()
        self.relu = nn.ReLU()
    ...
    def forward_geonetwork(self, inputs, steps=None):
        ...
        if self.config.frequency_regularizer and steps is not None:
            # current_step = kwargs['global_step'] % (self.opt.reload_epoch*1000)
            freq_mask = get_freq_reg_mask(self.config.posenc_len, steps, self.config.freq_reg_end, self.config.freq_reg_start)
            # print(h.shape) ([2097152, 32])
            freq_mask = freq_mask.repeat(pe.shape[0],1)
            pe = pe*freq_mask.to(pe.device)
    ...
    def get_sdf(self, ray_samples: RaySamples,steps=None):
        """predict the sdf value for ray samples"""
        positions = ray_samples.frustums.get_start_positions()
        positions_flat = positions.view(-1, 3)
        h = self.forward_geonetwork(positions_flat, steps=steps).view(*ray_samples.frustums.shape, -1)
    ...
    def get_density(self, ray_samples: RaySamples, steps=None):
    ...
    def get_alpha(self, ray_samples: RaySamples, sdf=None, gradients=None, steps=None):
        """compute alpha from sdf as in NeuS"""
        if sdf is None or gradients is None:
            inputs = ray_samples.frustums.get_start_positions()
            inputs.requires_grad_(True)
            with torch.enable_grad():
                h = self.forward_geonetwork(inputs, steps=steps)
                sdf, _ = torch.split(h, [1, self.config.geo_feat_dim], dim=-1)
    ...
    def get_alphabeta_nograd(self, ray_samples: RaySamples, sdf=None, gradients=None, steps=None):
        """compute alpha from sdf as in NeuS"""
        if sdf is None or gradients is None:
            inputs = ray_samples.frustums.get_start_positions()
            ...
            if self.config.detach_gradient:
                beta2 = self.uncertainty_net(geo_features.detach())
            else:
                beta2 = self.uncertainty_net(geo_features)
            beta2 = self.softplus_uncert(beta2)+self.config.beta_min
            d_output = torch.ones_like(sdf, requires_grad=False, device=sdf.device)
            gradients = torch.autograd.grad(
                outputs=sdf,
                inputs=inputs,
    ...
    def get_uncertainty(self, inputs=None, geo_features=None,
                         return_sdf=False, steps=None):
        if inputs is not None:
            # TODO check whether grad is necessary
            # with torch.enable_grad():
            h = self.forward_geonetwork(inputs, steps=steps)
            sdf, geo_features = torch.split(h, [1, self.config.geo_feat_dim], dim=-1)
        if geo_features is None:
            raise AttributeError("Inputs or geo_features should be provided.")
        if self.config.detach_gradient:
            beta2 = self.uncertainty_net(geo_features.detach())
        else:
            beta2 = self.uncertainty_net(geo_features)
    ...
    def get_colors(self, points, directions, gradients, geo_features, camera_indices, steps=None):
        """compute colors"""
        ...
        if self.config.frequency_regularizer and steps is not None:
            # current_step = kwargs['global_step'] % (self.opt.reload_epoch*1000)
            freq_mask = get_freq_reg_mask(self.config.direnc_len, steps, self.config.freq_reg_end, self.config.freq_reg_start)
            # print(h.shape) ([2097152, 32])
            freq_mask = freq_mask.repeat(d.shape[0],1)
            d = d*freq_mask.to(d.device)
    ...
    def get_outputs(self, ray_samples: RaySamples, return_alphas=False, 
                    return_occupancy=False, return_uncertainty=False, steps=None):
        ...
        if self.config.use_numerical_gradients:
            ...
            # inputs shape [65536, 3]
            # gradients shape [65536, 3]
            # sampled_sdf shape [6, 65536]
            # ray_samples.shape [512, 128]
            # ray_samples.frustums.directions.shape [512, 128, 3]
            if len(ray_samples.shape) <2:
                sampled_sdf = sampled_sdf.view(-1, *ray_samples.frustums.directions.shape[:-1]).permute(1, 0).contiguous()
            else:
        ...
        rgb = self.get_colors(inputs, directions_flat, gradients, geo_feature, camera_indices, steps=steps)
        ...
        if return_uncertainty:
            uncertainty = self.get_uncertainty(geo_features=geo_feature)
            outputs.update({FieldHeadNames.UNCERTAINTY: uncertainty})
