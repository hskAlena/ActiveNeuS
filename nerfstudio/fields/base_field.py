...
class Field(nn.Module):
    ...
    def density_fn(self, positions: TensorType["bs":..., 3],steps=None) -> TensorType["bs":..., 1]:
        ...
        density, _ = self.get_density(ray_samples,steps=steps)
        return density

    @abstractmethod
    def get_density(self, ray_samples: RaySamples,steps=None) -> Tuple[TensorType[..., 1], TensorType[..., "num_features"]]:
    ...
    def forward(self, ray_samples: RaySamples, compute_normals: bool = False, 
                compute_uncertainty: bool=False, steps=None):
        ...
        if compute_normals:
            with torch.enable_grad():
                density, density_embedding = self.get_density(ray_samples, steps=steps)
        elif compute_uncertainty:
            density, uncertainty, density_embedding = self.get_density(ray_samples, compute_uncertainty,steps=steps)
        else:    
            density, density_embedding = self.get_density(ray_samples,steps=steps)

        field_outputs = self.get_outputs(ray_samples, density_embedding=density_embedding,steps=steps)
        ...
        if compute_uncertainty:
            field_outputs[FieldHeadNames.UNCERTAINTY] = uncertainty
