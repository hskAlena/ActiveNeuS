...
ns-train active_neus-acc --pipeline.model.sdf-field.inside-outside False \
 ...
 --pipeline.model.overwrite_near_far_plane True \
    ...
    --experiment-name "neus-acc-entropy-topk-ent-2Labsph-5e5-${dataset}" \
    sdfstudio-data --data "data/dtu/${dataset}" \
    --train_val_no_overlap True --skip_every_for_val_split 10 \
    --indices_file "outputs/neus-acc-entropy-topk-ent-2Labsph-5e5-${dataset}/indices.txt"
done
wait
echo "Done."
...
for dataset in "${DATASETS[@]}"; do
 ...
 --pipeline.model.overwrite_near_far_plane True \
    ...
    --experiment-name "neus-acc-entropy-topk-20k1k-total10-2Labsph-5e5-${dataset}" \
    sdfstudio-data --data "data/dtu/${dataset}" --include_foreground_mask True \
    --indices_file "outputs/neus-acc-entropy-topk-20k1k-total10-2Labsph-5e5-${dataset}/indices.txt"
done
wait
echo "Done."

DATASETS=("scan24" "scan37" "scan40" "scan55" "scan63" "scan65" "scan69" "scan83" "scan97" "scan105" "scan106" "scan110" "scan114" "scan118" "scan122")
DATASETS=("scan97" "scan105" "scan106" "scan110" "scan63" )
for dataset in "${DATASETS[@]}"; do
ns-train active_neus-acc --pipeline.model.sdf-field.inside-outside False \
 ...
 --pipeline.model.acquisition random  --pipeline.model.overwrite_near_far_plane True \
      --pipeline.model.background_color="black" --pipeline.model.scene_contraction_norm inf \
      --pipeline.model.cone_angle 0.11 --pipeline.model.alpha_sample_thre 0.001 \
      --pipeline.model.near_plane 0.05 --pipeline.model.grid_levels 2 \
      --pipeline.model.sphere_masking True --pipeline.model.maintain_aabb False \
    ...
    --experiment-name "neus-acc-random-60k1k-Freq30k-diffinit-total10-2Labsph-5e5-${dataset}" \
    sdfstudio-data --data "data/dtu/${dataset}" \
    --indices_file "outputs/neus-acc-random-60k1k-Freq30k-diffinit-total10-2Labsph-5e5-${dataset}/indices.txt"
done
wait
echo "Done."
...
ns-train active_neus-acc --pipeline.model.sdf-field.inside-outside False \
 ...
 --pipeline.model.overwrite_near_far_plane True \
      ...
      --pipeline.model.sdf-field.freq_reg_end 30000 \
      --pipeline.model.sdf-field.frequency_regularizer True \
    --experiment-name "neus-acc-entropy-topk-60k1k-Freq30k-diffinit-total10-2Labsph-5e5-${dataset}" \
    sdfstudio-data --data "data/dtu/${dataset}" \
    --indices_file "outputs/neus-acc-entropy-topk-60k1k-Freq30k-diffinit-total10-2Labsph-5e5-${dataset}/indices.txt"
done
wait
echo "Done."