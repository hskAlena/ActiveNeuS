DATASETS=("mic" "ficus" "materials" "ship")
...
ns-train active-nerf-acc \
 ...
 --pipeline.model.grid_sampling True --pipeline.model.entropy_type no_surface \
    ...
    --experiment-name "nerf-acc-entropy-topk-100k1k-precrop6-Freq40k-total10-${dataset}" \
...
ns-train active-nerf-acc \
 ...
 --pipeline.model.grid_sampling True --pipeline.model.entropy_type no_surface \
    ...
    --experiment-name "nerf-acc-entropy-topk-200k2k-precropW12-total20-${dataset}" \
    blender-data --data "data/blender/${dataset}" --skip_every_for_val_split 8 \
