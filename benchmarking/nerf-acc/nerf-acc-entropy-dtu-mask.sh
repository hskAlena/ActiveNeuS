...
ns-train active-nerf-acc \
 ...
 --pipeline.model.entropy_type no_surface --pipeline.model.grid_sampling True \
    ...
    --experiment-name "nerf-acc-entropy-topk-60k1k-Freq30k-total10Mask-${dataset}" \
    sdfstudio-data --data "data/dtu/${dataset}" --include_foreground_mask True \
    --indices_file "outputs/nerf-acc-entropy-topk-60k1k-Freq30k-total10Mask-${dataset}/indices.txt"
done
wait
echo "Done."
...
ns-train active-nerf-acc \
 ...
 --pipeline.model.entropy_type no_surface --pipeline.model.grid_sampling True \
    ...
    --experiment-name "nerf-acc-entropy-topk-120k2k-total20Mask-${dataset}" \
    sdfstudio-data --data "data/dtu/${dataset}" --include_foreground_mask True \
    --indices_file "outputs/nerf-acc-entropy-topk-120k2k-total20Mask-${dataset}/indices.txt"
done
wait
echo "Done."
