...
ns-train active-nerf-acc \
 ...
 --pipeline.model.uncert_sigma_loss_mult 0.0 --pipeline.model.uncertainty_net True \
    ...
    --experiment-name "nerf-acc-active-topk-200k2k-precropW12-total20-${dataset}" \
...
ns-train active-nerf-acc \
 ...
 --pipeline.model.uncert_sigma_loss_mult 0.0 --pipeline.model.uncertainty_net True \
    ...
    --experiment-name "nerf-acc-active-topk-100k1k-precrop6-Freq40k-total10-${dataset}" \
    blender-data --data "data/blender/${dataset}" --skip_every_for_val_split 8 \
    --precrop-iters 1275000 --precrop-frac 0.6
done
wait
echo "Done."